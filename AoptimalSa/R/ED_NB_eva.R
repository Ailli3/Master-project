...
find_adjacent_pairs <- function(data){
  D <- data %>%
    ...
    select(-row) %>%
    as.matrix()
  De <- rbind(NA, cbind(NA, D, NA), NA)
  num_rows <- nrow(D)
  num_cols <- ncol(D)
  adjacent_pairs <- lapply(list(c(0, 1), c(1, 1), c(1, 0), c(0, -1), c(-1, 0), c(-1, -1), c(1, -1), c(-1, 1)), function(neighbour) {
    data.frame(pos = as.vector(D),
               neighbour = as.vector(De[1 + c(1:num_rows) + neighbour[1], 1 + c(1:num_cols) + neighbour[2]])) |>
      na.omit()
  }) |>
    dplyr::bind_rows()

  return(table(adjacent_pairs$pos, adjacent_pairs$neighbour))
}
