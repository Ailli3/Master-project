...
Linear (mixed) models are the primarly method of analysis of experimental data with response that have an approximate Normal distribution. In this chapter, we expand on the model 
...
where $\boldsymbol{V} = \boldsymbol{Z}\boldsymbol{G}\boldsymbol{Z}^\top + \boldsymbol{R}$. In this paper, we primarily discuss the treatment effect, so we provide the proof of the treatment effect.
...
$$
\mathbf{C}_{lmm} = -\mathbb{E}\left(\frac{\partial^2}{\partial\boldsymbol{\tau}\partial\boldsymbol{\tau}^\top}\log\ell(\boldsymbol{\tau};\boldsymbol{y})\right)
$$
:::

And we have $\boldsymbol{\tau}\sim N(\hat{\boldsymbol{\tau}},\boldsymbol{C}_{lmm}^{-1})$.We will provide the specific form of this matrix and the related proof in the introduction of the A-value.
...
::: proof
From @eq-ll and basing in assumption we have
$$
