...
A row-column design refers to an experiment where experimental units are defined by the Cartesian product of two sets, denoted as $\mathcal{R}$ and $\mathcal{C}$, respectively, such that $\Omega = \mathcal{R} \times \mathcal{C}$. We denote $F_R$ and $F_C$ as the surjective functions that map $\Omega$ to $\mathcal{R}$ and $\mathcal{C}$, respectively. We use $V_R$ and $V_C$ to denote vector spaces in $\mathbb{R}^N$ for row and column factors, respectively.

:::


In a row-column design, the row and column factors are often considered as nuisance (or blocking) factors. The treatments of interest are then randomly allocated with certain constraints as to reduce confounding the treatment factors with these nuisance factors. An example of row-column design is the _Latin square design_ where $n_r = n_c$ and each treatment appears exactly once in each row and column (see @exm-lsd). 
