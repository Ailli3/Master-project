...
Linear (mixed) models are the primarly method of analysis of experimental data with response that have an approximate Normal distribution. In this chapter, we expand on the model @eq-lm to consider linear mixed models for the analysis of experimental data with a more general experimental structure.


## Linear mixed model

Linear mixed model incorporates additional random effects into the model that effectively give
greater flexibility and capability to incorporate known correlated
structures into the model. We consider a linear mixed model $$
\boldsymbol{y}=\boldsymbol{X}\boldsymbol{\tau}+\boldsymbol{Z}\boldsymbol{u}+\boldsymbol{\epsilon}
$$ {#eq-lmm} where $\boldsymbol{y}$ is a $n\times 1$ vector of
observations, $\boldsymbol{\tau}$ is a $t\times1$ vector of
fixed effects, $\boldsymbol{u}$ is a $q \times1$ vector of
random effects, and $\boldsymbol{\epsilon}$ is the $n\times 1$ error
vector, $\boldsymbol{X}$ and $\boldsymbol{Z}$ are design matrices of
dimensions $n \times t$ and $n \times q$ for fixed and random effects, respectively. We assume that 
$$
\begin{bmatrix}
\boldsymbol{u} \\
\boldsymbol{\epsilon} 
...
\end{bmatrix}
\right),
$$ {#eq-dist} where $\boldsymbol{G}$ is a $q \times q$ variance matrix for
$\boldsymbol{u}$ and $\boldsymbol{R}$ is a $n\times n$ variance matrix for
$\boldsymbol{\epsilon}$. 
...
$$(\boldsymbol{W}^\top\boldsymbol{R}^{-1}\boldsymbol{W} + \boldsymbol{G}^*)\tilde{\boldsymbol{\beta}} = \boldsymbol{W}\boldsymbol{R}^{-1}\boldsymbol{y},$$ where $\boldsymbol{W} = \begin{bmatrix}\boldsymbol{X} & \boldsymbol{Z} \end{bmatrix}$, $\tilde{\boldsymbol{\beta}} = (\hat{\boldsymbol{\tau}}^\top, \tilde{\boldsymbol{u}}^\top)^\top$ and $\boldsymbol{G}^* = \begin{bmatrix}\boldsymbol{0}_{p\times p} & \boldsymbol{0}_{p\times q} \\ \boldsymbol{0}_{q\times p} & \boldsymbol{G}^{-1}\end{bmatrix}$.
...
\tilde{\boldsymbol{u}} &= \boldsymbol{G}\boldsymbol{Z}^\top\boldsymbol{V}^{-1}(\boldsymbol{y} - \boldsymbol{X}\hat{\boldsymbol{\tau}}),
\end{aligned}$$

where $\boldsymbol{V} = \boldsymbol{Z}\boldsymbol{G}\boldsymbol{Z}^\top + \boldsymbol{R}$.
...
The log-likelihood of @eq-lmm is then given as:
...
$$ and thus $\mathbf{C} = \frac{1}{\sigma^2}\boldsymbol{X}^\top\boldsymbol{X}$.
:::

::: {#lem-lm-var}
The variance of the estimate of fixed effects for @eq-lm is equivalent to the
